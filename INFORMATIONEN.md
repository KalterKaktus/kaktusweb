...
- Karl die Schildkröte spawnen (`spawn-karl`) — Bonus-Event, Spieler kann Panzer sauber putzen und am Glücksrad drehen für Coins oder Force-Spawns. Auto-spawn max. 1× pro Stunde via deterministischem Hash. Reward-Konfig in `js/data/karl.js` (`KARL_REWARDS_BY_AREA`), System in `js/systems/karlSystem.js`. Karl-PNG in `assets/karl.png`.
- Daily Login-Rewards in `js/systems/dailySystem.js` + Schedule in `js/data/dailyRewards.js`. State unter `state.daily = { streak, lastClaimedDay, totalClaimed, bestStreak }`. Streak basiert auf lokalem Datum, resettet bei Day-Gap > 1. Reward-Skala: Woche 1 Tag 7 = Epic, Woche 2 Tag 7 = 2× Epic, ab Woche 3 Tag 7 = Legendary. Coin-Base wächst 12 %/Woche, gedeckelt bei 2500 Basis-Coins (Tag 7 = ×8). Daily-Spawn-Spots leben 90 s (statt 12 s) damit niemand seine garantierte Belohnung verpasst.
- Fish-Index: 132 Fische total (Pond 36, Lake 44, Ocean 52). Render-Pattern: nur die aktuell aktive Area zeigt SVG-Cards, andere Areas zeigen nur den Counter + Hint („Wechsel zu X" / „Durch Prestige gesperrt"). Massive Perf-Win vs. alle SVGs gleichzeitig zu rendern. `FISH_POOL_WEIGHTS` und `FISH_VALUE_MULTIPLIERS` unterstützen jetzt Pool-Größen bis 16 (siehe `js/data/rarities.js`).
- Angel-UI in `js/systems/angelUiSystem.js`: ersetzt den klassischen Shop. Window `data-window="shop"` wird intern behalten, Label im UI ist „Angel". Render-Logik: SVG-Angel oben (Parts ändern Farbe/Glow pro Upgrade-Level), Part-Tabs (Rute/Schnur/Haken/Köder/Glück), Detail-Panel mit Level-Dots, Beschreibung, Buy-Button. Mapping Part→Upgrade in `PART_TO_UPGRADE` (Köder = `sonar` intern). Visuals via Color-Arrays (`ROD_COLORS`, `LINE_COLOR` etc.) und Shape-Branches per Level. Game-Windows leben jetzt im Grid-Slot 3 der `.fishing-shell` zusammen mit `.water-stage` → Nav bleibt klickbar, Wasser wird komplett bedeckt.
